...
def save_conversion_history(stats_run, run_dir: Path) -> Tuple[Path, Path]:
    ...
    Formato esperado real:
        [(t, None, conv), ...]
    aunque también se tolera:
        [(t, conv), ...]
    """
    conv_hist = stats_run["conversion_history"]

    if not conv_hist:
        raise ValueError("stats_run['conversion_history'] está vacío.")

    first_item = conv_hist[0]

    # Caso 1: (t, None, conv) o (t, algo, conv)
    if len(first_item) >= 3:
    ...
    # Caso 2: (t, conv)
        ...
        raise ValueError(
            "Formato no reconocido en stats_run['conversion_history']. "
...
def run_single_sigma_simulation(
    ...
    Lat.initialize(mode="random", max_height=1, n_seeds=n_seeds)
